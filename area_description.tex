...
\emph{
	...
	\node (conclusion2) [above right=0.1cm and 0.3cm of text2.north west] {CONCLUSION 1};
	\node (premise2) [above right=0.1cm and 0.5cm of text2.north] {PREMISE 1};
	}
...
\caption{Example of two comments analyzed using an argumentation mining pipeline. 
	...
	\hlgreen{premises}. Then \textcolor{darkgreen}{support} relations are
...
% - if this is compared to a more formal surrounding, such as the ones seen in computational 
% argumentation, it can be observed that this kind of language is far more
% ``noisy'', ``in the wild'', which emphasizes the challenges seen in
% argumentation mining \\



% - an example of a post on an internet discussion in marijuana:  \\
% - ``\emph{\textbf{I absolutely disagree with legalization of Marijuana!!!!}
% <S2>It is nonsence,</S2> <S3>\textbf{because Marijuana is the drugs</S3> and <S4>has
% "negative health effect</S4>} is <S5>the long term effect on the hippocampus
% which retains memory in the brain.</S5> <S6>Research has shown that excessive
% marijuana smoking can cause damage to the brain"</S6>
% (\url{http://futurepocket.com/2011/02/19/ why-the-government-wont-legalize-
% marijuana/}) <S7>Think about our children!</S7> <S8>I live in Kazakhstan, there
% is a suburb, the place where Marijuana grows and takes huge territory, As a
% result here is a lot of idiots because of constant using.</S8>
% }''
% - here we bold the main stance of the claim and the main reason behind the stance \\
% - there are other claims made in the post which further clarify the author's stance
% towards the topic of marijuana legalization \\
% - if this is compared to a more formal surrounding, such as the ones seen in computational 
% argumentation, it can be observed that this kind of language is far more
% ``noisy'', ``in the wild'', which emphasizes the challenges seen in
% argumentation mining \\
% - if we proclaim that:
% \begin{itemize}
% \item \textbf{C1: I absolutely disagree with legalization of Marijuana!!!!}
% \item \textbf{C2: because Marijuana is the drugs}
% \end{itemize}
% the goal of argumentation mining is to determine the relationship between these two claims
% \\
% - as said, usually, these relationships are defined as support, attack, and neutral relations 
% \citep{boltuzic2014back} \\
% - in this case, we infer that this is indeeed a support relationship
% 
% - extremely complex as it involves knowledge represenatation \\
...
Computational argumentation studied three different types of models of argumentation:
...
Argumentation is structured through different \emph{argumentation models}. One
...
The input for classifiers is based on features, which were mostly built from input text.
...
\paragraph{Argumentative boundary detection.}
...
\paragraph{Argument model. } Most research adopts some variation of
...
\paragraph{Wikipedia.} IBM has founded argumentation research projects that are dedicated 
...
% TODO implicit arguments are mentioned in 
% \citep{park2015conditional}
